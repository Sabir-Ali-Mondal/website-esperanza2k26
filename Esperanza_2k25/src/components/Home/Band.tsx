    ...
    <section ref={sectionRef} className="py-16 px-8 max-w-6xl mx-auto">
      ...
      <div className="space-y-16">
        {loading ? (
          ...
          bands.map((bandItem) => (
            <div key={bandItem._id} className="relative flex flex-col items-center">
              {/* Band title and description grouped together */}
              <div className="text-center mb-8 max-w-2xl">
                <h3 className={`text-4xl md:text-5xl font-bold text-white mb-4 ${sedgwick.className}`}>
                  {bandItem.title}
                </h3>
                {bandItem.description && (
                  <p className={`${sedgwick.className} text-base md:text-lg text-gray-200 leading-relaxed underdog`}>
                    {bandItem.description}
                  </p>
                )}
              </div>
