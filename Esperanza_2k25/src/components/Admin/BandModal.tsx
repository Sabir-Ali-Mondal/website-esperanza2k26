...
interface BandModalProps {
  ...
  onClose: () => void;
  band?: Band | null;
  onSave: (data: Partial<Band>) => Promise<void>;
}

export default function BandModal({ isOpen, onClose, band, onSave }: BandModalProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formData.append("folder", "esperanza2k26/bands");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title?.trim()) {
      customSwal.fire("Error", "Please enter a band title", "error");
      return;
    }
    if (!formData.imageUrl?.trim()) {
      customSwal.fire("Error", "Please upload an image", "error");
      return;
    }
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-start gap-4">
                  {formData.imageUrl && (
                    <div className="w-40 h-28 rounded-xl overflow-hidden border border-gray-700 bg-gray-800">
                      <img
